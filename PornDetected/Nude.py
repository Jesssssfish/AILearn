...
# @Describe: 色情图片鉴别,效果不理想，该demo主要在于python学习与应用
...
class Nude(object):
    ...
    # 需要传递一个类型名和你需要的字段给它，然后它就会返回一个类，你可以初始化这个类，为你定义的字段传递值等。
    Skin = namedtuple("Skin", "id skin region x y")

    def __init__(self, path_or_image):
        # 如果path_or_image是Image.Image类型的实例,直接赋值
        if isinstance(path_or_image, Image.Image):
            self.image = path_or_image
        # 如果path_or_image为str类型的实例，打开图片
        elif isinstance(path_or_image, str):
            self.image = Image.open(path_or_image)

        # 获得图片所有颜色通道
        bands = self.image.getbands()
        # 判断是否为单通道图片（也即灰度图），是则将灰度图转为RGB图
        if len(bands) == 1:
            new_image = Image.new("RGB", self.image.size)
            # 拷贝灰度图self.image到RGB图 new_image.paste(PIL自动进行颜色通道转换)
       ...
       """
        ...
        if maxwidth:
            if self.width > maxwidth:
                ...
                hszie = int(self.height * wpercent)
                fname = self.image.filename
                # Image.LANCZOS是重采样滤波器,用于抗锯齿
                self.image = self.image.resize((maxwidth, hszie), Image.LANCZOS)
        ...
        if maxheight:
            if self.height > maxheight:
                ...
                fname = self.image.filename
                self.image = self.image.resize((wsize, self.height), Image.LANCZOS)
                self.image.filename = fname
                self.width, self.height = self.image.size
                self.total_pixels = self.width * self.height
                ret += 2
    ...
    def parse(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                # 为每个像素创建一个对应的Skin对象,并添加到self.skiin_map中
                ...
                check_indexes = [_id - 2,  # 当前像素左方的像素
                                 _id - 2 - self.width,  # 当前像素左上方的像素
                                 _id - 1 - self.width,  # 当前像素上方的像素
                                 _id - self.width]  # 当前像素右上方的像素
                # 用来记录相邻像素中肤色像素所在区域号，初识化为-1
                region = -1
                # 遍历每一个相邻像素的索引
                for index in check_indexes:
                    ...
                    if self.skin_map[index].skin:
                        # 若相邻像素与当前像素的region均为有效值，且二者不同，且尚未添加到相同合并任务
                ...
                # region不等于-1的同时不等于None，说明有区域号为有效值的相邻肤色像素
                elif region != None:
                    # 将此像素的区域号更改为与相邻像素相同
